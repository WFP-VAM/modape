...
from modape.modis.smooth import ModisSmoothH5
...
class TestConsoleScripts(unittest.TestCase):
    ...
    def test_modis_download(self):
        ...
        with patch(
            "modape.scripts.modis_download.ModisQuery", return_value=test_query
        ) as mocked_query:
            ...
            result = self.runner.invoke(
                modis_download_cli,
                ["MOD13A2", "--username", "test", "--password", "test", "--download"],
            )
            mocked_query.assert_called()

            mocked_query.reset_mock()

            fake_hdf = self.testpath.joinpath(
                "MOD13A2.A2002193.h18v06.006.2019256103823.hdf"
            )
            fake_hdf.touch()

            result = self.runner.invoke(
                modis_download_cli, ["MOD13A2", "--targetdir", str(self.testpath)]
            )
            mocked_query.assert_called()
            assert result.exit_code == 0
            ...
            result = self.runner.invoke(
                modis_download_cli,
                ["MOD13A2", "--targetdir", str(self.testpath), "--target-empty"],
            )
            mocked_query.assert_not_called()
            ...
            result = self.runner.invoke(
                modis_download_cli, ["MOD13A2", "--roi", "10,10"]
            )
            mocked_query.assert_called()
            args = mocked_query.call_args_list[0][1]
            ...
            result = self.runner.invoke(
                modis_download_cli, ["MOD13A2", "--roi", "10,10,20,20"]
            )
            mocked_query.assert_called()
            args = mocked_query.call_args_list[0][1]
            ...
            result = self.runner.invoke(
                modis_download_cli, ["MOD13A2", "--roi", "10,20,20"]
            )
            mocked_query.assert_not_called()
            assert result.exit_code == 1
            ...
            result = self.runner.invoke(
                modis_download_cli, ["MOD13A2", "--roi", "10,20,20,30,50"]
            )
            mocked_query.assert_not_called()
            assert result.exit_code == 1
    ...
    def test_modis_collect(self):
        ...
        with patch("modape.scripts.modis_collect._worker") as mocked_worker:
            ...
            result = self.runner.invoke(
                modis_collect_cli, ["/tmp/data", "--interleave", "--cleanup"]
            )
        ...
        with patch("modape.scripts.modis_collect._worker") as mocked_worker:
            ...
            with patch("modape.scripts.modis_collect.ModisRawH5") as mocked_rawfile:
                mocked_rawfile.return_value = MagicMock(
                    exists=True, last_collected="2020001"
                )
                result = self.runner.invoke(
                    modis_collect_cli,
                    ["/tmp/data", "--interleave", "--last-collected", "2020365"],
                )
                ...
                result = self.runner.invoke(
                    modis_collect_cli,
                    ["/tmp/data", "--interleave", "--last-collected", "2020001"],
                )
            ...
            result = self.runner.invoke(
                modis_collect_cli, ["/tmp/data", "--interleave"]
            )
            ...
            result = self.runner.invoke(
                modis_collect_cli,
                ["/tmp/data", "--interleave", "--tiles-required", "h18v06,h19v06"],
            )
            self.assertEqual(result.exit_code, 0)
            self.assertEqual(mocked_worker.call_count, 2)
    ...
    def test_modis_smooth(self):
        ...
        with patch(
            "modape.scripts.modis_smooth._worker", return_value=True
        ) as mocked_worker:
            result = self.runner.invoke(
                modis_smooth_cli, [str(raw_h5), "--targetdir", str(raw_h5)]
            )
            ...
            result = self.runner.invoke(
                modis_smooth_cli, [str(raw_h5), "--nsmooth", 1, "--nupdate", 10]
            )
            ...
            result = self.runner.invoke(
                modis_smooth_cli, [str(raw_h5), "--srange", 0, 1]
            )
            ...
            result = self.runner.invoke(
                modis_smooth_cli, [str(raw_h5), "--srange", 0, 1, 0.2]
            )
        ...
        with patch(
            "modape.scripts.modis_smooth._worker", return_value=False
        ) as mocked_worker:
        ...
        with patch(
            ...
            np.testing.assert_array_equal(
                mkwargs["srange"], np.arange(0, 1.2, 0.2).round(2)
            )
        ...
        with patch("modape.scripts.modis_smooth.ModisSmoothH5") as mocked_smoothfile:
            ...
            mocked_smoothfile.return_value = MagicMock(
                exists=True, last_collected="2020001"
            )
            result = self.runner.invoke(
                modis_smooth_cli, [str(raw_h5), "--last-collected", "2020002"]
            )
            mocked_smoothfile.assert_called_once()
            self.assertEqual(result.exit_code, 1)
            ...
            result = self.runner.invoke(
                modis_smooth_cli, [str(raw_h5), "--last-collected", "2020001"]
            )
            mocked_smoothfile.assert_called_once()
            self.assertEqual(result.exit_code, 0)
    ...
    def test_modis_window(self, mocked_mosaic):
        ...
        self.assertEqual(
            mkwargs["creationOptions"], ["COMPRESS=DEFLATE", "PREDICTOR=1", "TILED=YES"]
        )
        self.assertEqual(mkwargs["clip_valid"], False)
        ...
        with patch(
            "modape.scripts.modis_window.ModisMosaic.__init__", return_value=None
        ) as mock_init:
    ...
    def test_collect_smooth_export_import_sgrid(self):
        """
        This is more an integration test, really...

        The following is tested:

            1. (a) `modis_collect` will find 16 HDF-files: 15x for h18v09, 1x for h18v08
               (b) Parameter `tiles_required` reads only "h18v09", thus h18v08 will not qualify:
                => for this tile the H5-file is merely initialized: no data written
            2. `modis_smooth` will intialize the smoothing grid (sgrid) and produce 12-timestep
                smoothed H5-file
            3. `modis_window` is invoked to export the sgrid
            4. `modis_collect`: ingest for forward processing
            5. Do forward processing and export dekads
            6. remove SMOOTH .h5 archives
            7. Initialize new SMOOTH .h5 archives by importing from sgrid
            8. Do forward processing and export dekads in same fashion as (5)


        """

        tile_dates_len: dict[str, dict[str, int]] = {
            "h18v08": dict(VIM=0, SMOOTH=0),
            "h18v09": dict(VIM=15, SMOOTH=12),
        }
        sgrid_hashes: dict[str, str] = {
            "h18v08": "f3f13825d8e604a5154fafba39542fbe99b75de1",
            "h18v09": "cbdb80c7951203dd911e8996fef9e720cc046a96",
        }
        dekad_hashes: dict[str, str] = {
            "202505d2": "3cb5c6e47c091b18c5ed922fc0114df9b7e40ec6",
            "202505d3": "4396d68dd01234c7fc9a4a85fe96282f5dc64881",
        }

        def smooth_and_export():
            modis_smooth_cli.callback(
                src=Path(temp_dir).joinpath("VIM").as_posix(),
                targetdir=Path(temp_dir).joinpath("VIM", "SMOOTH").as_posix(),
                svalue=None,
                srange=None,
                pvalue=None,
                tempint=10,
            ...
            modis_window_cli.callback(
                src=Path(temp_dir).joinpath("VIM", "SMOOTH").as_posix(),
                targetdir=Path(temp_dir).joinpath("VIM", "SMOOTH", "EXPORT").as_posix(),
                begin_date=datetime(2025, 5, 15),
            ...
            for dekad in ["202505d2", "202505d3"]:
                ds: gdal.Dataset = gdal.Open(
                    Path(temp_dir)
                    .joinpath("VIM", "SMOOTH", "EXPORT", f"TESTvim{dekad}.tif")
                    .as_posix()
                )
                try:
                    self.assertEqual(
                    ...
                    Path(temp_dir).joinpath(
                        "VIM", "SMOOTH", "EXPORT", f"TESTvim{dekad}.tif"
                    ).unlink()

        with tempfile.TemporaryDirectory() as temp_dir:
            src = Path(__file__).resolve().parent.joinpath("data", "init")
            files = [file.as_posix() for file in src.glob("*.hdf")]

            # 1. `modis_collect` will find 16 HDF-files: 15x for h18v09, 1x for h18v08:
            modis_collect_cli.callback(
                src_dir=src.as_posix(),
                targetdir=temp_dir,
                compression="gzip",
                vam_code="VIM",
                interleave=True,
            ...
            for tile, dates_len in tile_dates_len.items():
                h5 = ModisRawH5(
                    [
                        src.joinpath(file).as_posix()
                        for file in files
                        if file.find(f".{tile}.") > -1
                    ],
                    temp_dir,
                    "VIM",
                    True,
                )
            with h5py.File(h5.filename, "r+", libver="latest") as h5f:
                dates = h5f.get("dates")
                self.assertEqual(len(dates), dates_len["VIM"])

            # 2. `modis_smooth` will intialize the smoothing grid (sgrid)
            modis_smooth_cli.callback(
                src=Path(temp_dir).joinpath("VIM").as_posix(),
                targetdir=Path(temp_dir).joinpath("VIM", "SMOOTH").as_posix(),
                svalue=None,
            ...
            for rawfile in Path(temp_dir).joinpath("VIM").glob("*.h5"):
                tile = REGEX_PATTERNS["tile"].findall(rawfile.name)[0]
                smt_h5 = ModisSmoothH5(
                    rawfile=rawfile.as_posix(),
                    targetdir=Path(temp_dir).joinpath("VIM", "SMOOTH").as_posix(),
                    tempint=10,
                )
                with h5py.File(smt_h5.filename, "r+", libver="latest") as h5f:
                    dates = h5f.get("dates")
                    self.assertEqual(
                        hashlib.sha1(h5f.get("sgrid")[:].data.tobytes()).hexdigest(),
                        sgrid_hashes[tile],
                    )
                    self.assertEqual(len(dates), tile_dates_len[tile]["SMOOTH"])
            ...
            # 4. `modis_collect`: ingest for forward processing
            src = Path(__file__).resolve().parent.joinpath("data", "fwd")
            files = [file.as_posix() for file in src.glob("*.hdf")]
            ...
            # 6. remove SMOOTH .h5 archives
            for _archive in Path(temp_dir).joinpath("VIM", "SMOOTH").glob("*.h5"):
                _archive.unlink()
            ...
            modis_smooth_cli.callback(
                ...
                soptimize=False,
                sgrid=Path(temp_dir)
                .joinpath("VIM", "SMOOTH", "EXPORT", "TESTvim_sgrid.tif")
                .as_posix(),
                parallel_tiles=1,
                last_collected=None,
            )
            ...
            for _archive in Path(temp_dir).joinpath("VIM", "SMOOTH").glob("*.h5"):
                tile = REGEX_PATTERNS["tile"].findall(_archive.name)[0]
                with h5py.File(_archive, "r+", libver="latest") as h5f:
                    self.assertEqual(
                        hashlib.sha1(h5f.get("sgrid")[:].data.tobytes()).hexdigest(),
                        sgrid_hashes[tile],
                    )
    ...
    @patch(
        "modape.scripts.csv_smooth.ws2doptvp", return_value=(np.random.rand(100), 10)
    )
    ...
    def test_csv_smooth(self, mock_ws2d, mock_ws2doptv, mock_ws2doptvp):
        ...
        _ = pd.DataFrame(
            {"TS1": np.random.rand(100), "TS2": np.random.rand(100)}
        ).to_csv(testfile)
        ...
        result = self.runner.invoke(
            csv_smooth_cli, [testfile, "--soptimize", "--srange", "-2.0", "2.0", "0.2"]
        )
        mock_ws2doptv.assert_called()
        margs, _ = mock_ws2doptv.call_args
        self.assertEqual(result.exit_code, 0)
        ...
        result = self.runner.invoke(
            csv_smooth_cli, [testfile, "--soptimize", "-p", "0.90"]
        )
