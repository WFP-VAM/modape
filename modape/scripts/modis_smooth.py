...
@click.option(
    "--tempint-start",
    type=click.DateTime(formats=["%Y-%m-%d", "%Y%j"]),
    help="Startdate for temporal interpolation",
)
@click.option(
    "-n", "--nsmooth", type=click.INT, default=0, help="Number of raw timesteps used for smoothing"
)
@click.option(
    "-u",
    "--nupdate",
    type=click.INT,
    default=0,
    help="Number of smoothed timesteps to be updated in HDF5 file",
)
@click.option("--soptimize", is_flag=True, help="Use V-curve for s value optimization")
@click.option(
    "--sgrid",
    type=click.Path(dir_okay=False, resolve_path=True),
    help="File to import previously exported S-grid from",
)
@click.option(
    "--parallel-tiles", type=click.INT, help="Number of tiles processed in parallel", default=1
)
@click.option(
    "--last-collected",
...
def cli(
    ...
    Args:
        ...
        sgrid (str): File to import previously exported S-grid from.
    ...
    if sgrid is not None:
        click.echo("Starting MODIS SMOOTH S-grid IMPORT!")
        ...
        for rawfile in files:
            ...
            smt_h5.write_sgrid(sgrid)

        click.echo("MODIS SMOOTH S-grid import COMPLETED!")
        return
    ...
    if parallel_tiles > 1:
        ...
        for rawfile in files:
            ...
            result = _worker(
                rawfile, targetdir, tempint, tempint_start, last_collected, **smoothing_parameters
            )
...
def _worker(
    ...
    **kwargs: dict,
    ...
    if last_collected is not None:
        ...
        assert (
            last_collected == last_collected_in_rawfile
